...
def associate_by_email(backend, details, user=None, *args, **kwargs):
    ...
    try:
        ...
        # Verifica se o usuário já está associado ao Google
        social_auth = UserSocialAuth.objects.filter(user=existing_user, provider='google').first()
        if social_auth:
            # Se o usuário já estiver associado ao Google, faça login normalmente
            login(backend.strategy.request, existing_user, backend='social_core.backends.google.GoogleOAuth2')
            return {'user': existing_user, 'redirect': reverse('home')}
        else:
            # Se o usuário não estiver associado ao Google, crie a associação
            backend.strategy.storage.user.create_social_auth(user=existing_user, provider='google', uid=details.get('uid'))
            login(backend.strategy.request, existing_user, backend='social_core.backends.google.GoogleOAuth2')
        ...
        if email:
            ...
            backend.strategy.storage.user.create_social_auth(user=user, provider='google', uid=details.get('uid'))
